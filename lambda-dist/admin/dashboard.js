...
export const dashboardHandler = async () => {
    const [totalCustomers, activeSubscriptions, totalRevenue, pendingPayments, recentPayments,] = await Promise.all([
...
function calculateMRR(subscriptions) {
    ...
    for (const subscription of subscriptions) {
        ...
        const addonPrices = subscription.addons.reduce((sum, sa) => sum + Number(sa.addon.price) * sa.quantity, 0);
