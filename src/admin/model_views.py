from src.admin.sqladmin_config import SecureModelView as SecureModelView
...
class PlanAdmin(SecureModelView, model=Plan):
    ...
    column_list = [Plan.id, Plan.name, Plan.cycle, Plan.is_active, Plan.created_at]
    ...
    form_args = {
        "features": {"label": "Features (JSON)", "description": "Enter features as a JSON array"}
    }
...
class AddonAdmin(SecureModelView, model=Addon):
    ...
    column_list = [Addon.id, Addon.name, Addon.price, Addon.type, Addon.is_active, Addon.created_at]
...
class WebhookLogAdmin(SecureModelView, model=WebhookLog):
    ...
    column_list = [WebhookLog.id, WebhookLog.event, WebhookLog.success, WebhookLog.processed_at]
