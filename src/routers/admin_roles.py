...
from src.admin.auth import get_current_user, require_permission
...
async def get_available_permissions(
    ...
    for group_name, permissions in permission_groups.items():
        result[group_name] = [
            {
                "value": perm.value,
                "description": PERMISSION_DESCRIPTIONS.get(perm, perm.value)
            }
...
async def get_role_users(
    ...
    role_exists = await db.execute(
        select(exists().where(AdminRole.id == role_id))
    )
